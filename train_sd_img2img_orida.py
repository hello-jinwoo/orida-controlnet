...
# ##### Customized Part #####
# from pipelines.custom_pipeline_stable_diffusion_inpaint import CustomStableDiffusionInpaintPipeline
...
def log_validation(
    ...
    # )
    pipeline = StableDiffusionImg2ImgPipeline.from_pretrained( # 상민
    ...
    # pipeline.scheduler = UniPCMultistepScheduler.from_config(pipeline.scheduler.config) # 상민
...
def main(args):
    ...
    print("Load textencoder " * 20)

    # import correct text encoder class
    text_encoder_cls = import_model_class_from_model_name_or_path(args.pretrained_model_name_or_path, args.revision)

    print("Finish Load text encoder & Load Models " * 20)
    ...
    print(">> Making train dataset " * 20)

    train_dataset = make_train_dataset(args, tokenizer, accelerator)

    print(">> Finish Making train dataset " * 20)
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(unet):
                ...
                latent_model_input = noisy_latents # 상민
                model_pred = unet(
                    latent_model_input,
                    timesteps,
